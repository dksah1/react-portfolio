...
const WorkItem = ({imgUrl,title,tech,workUrl}) => {
  return (
<a href={workUrl} target='_blank' className="bg-slate-300 dark:bg-slate-800 rounded-lg
overflow-hidden ">
   ...
   items-center justify-center text-xs md:text-sm '>
    {
        tech.map(item=>(
            <span key={item} className='inline-block px-2 py-1 
            bg-slate-200 dark:bg-slate-900 rounded-md'>
                {item}&nbsp;
            </span>
        ))
    }
